...
const getItemFile = async (item, tries = 0) => {
  ...
  if (cache.isBusy(ITEMS_DIR, filename)) {
    console.error(`${filename} is busy. Retrying...`)
    await sleep(5000);
  ...
  try {
    ...
    console.error(`Failed to create writestream for ${filename}:`, e);
    await sleep(5000);
...
async function getPlayer(playerId, { server = SERVER_DEFAULT, silent = false }) {
  try {
    console.log(`Fetch ${server.name} player: ${playerId}`);
    const player = await albionApiClient.getPlayer(playerId, { server });
    player.server = server;
    return player;
  } catch (error) {
    if (!silent)
      console.error(`Failed to fetch ${server.name} player [${playerId}]: ${error.message}`, {
        error,
        server,
        silent,
        playerId,
...
async function getGuild(guildId, { server = SERVER_DEFAULT, rankings = false, silent = false }) {
  try {
    ...
    if (!silent)
      console.error(`[${server.name}] Failed to fetch guild [${guildId}]: ${error.message}`, {
        error,
        server,
        silent,
        guildId,
...
async function getAlliance(allianceId, { server = SERVER_DEFAULT, silent = false }) {
  try {
    console.log(`Fetch ${server.name} alliance: ${allianceId}`, { allianceId, server, silent });
    const alliance = await albionApiClient.getAlliance(allianceId, { server });
    alliance.server = server;
    return alliance;
  } catch (error) {
    if (!silent)
      console.error(`[${server.name}] Failed to fetch alliance [${allianceId}]: ${error.message}`, {
        error,
        server,
        silent,
        allianceId,
...
async function search(query, { server = SERVER_DEFAULT }) {
  try {
    console.log(`Searching entities in ${server.name} for: ${query}`, { query, server });
    return await albionApiClient.search(query, { server });
  } catch (error) {
    console.error(`[${server.name}] Failed to search query [${query}] in API: ${error.message}`, {
      error,
      query,
      server,
    });
...
async function guildByName(query, { server = SERVER_DEFAULT }) {
  try {
    console.log(`Searching guild in ${server.name} for: ${query}`);
    return await albionApiClient.guildByName(query, { server });
  } catch (error) {
    console.error(`[${server.name}] Failed to search query [${query}] in API: ${error.message}`, {
      error,
      query,
...
async function guildMembers(guildId, { server = SERVER_DEFAULT }) {
  try {
    console.log(`Fetch ${server.name} guild: ${guildId}`, { server });
    const guild = await albionApiClient.guildMembers(guildId, { server });
...
async function guildMemberKillsById(memberId, { server = SERVER_DEFAULT }) {
  try {
    const kills = await albionApiClient.guildMemberKillsById(memberId, { server });

    return kills;
  } catch (error) {
    if (!silent)
      console.error(`[${server.name}] Failed to fetch member kills [${memberId}]: ${error.message}`, {
        error,
        server,
        memberId,
      });
...
async function guildMemberDeathsById(memberId, { server = SERVER_DEFAULT }) {
  try {
    const kills = await albionApiClient.guildMemberDeathsById(memberId, { server });

    return kills;
  } catch (error) {
    if (!silent)
      console.error(`[${server.name}] Failed to fetch member deaths [${memberId}]: ${error.message}`, {
        error,
        server,
        memberId,
      });
...
const useCurrentLootValue = async (server, itemList, qualities) => {
  const itemPriceData = await albionDataApiClient.getCurrentPrices(itemList, {
    server,
  ...
  return memoize(
    ...
    async () => {
      try {
        ...
        console.error(`[${server.name}] Failed to fetch kill loot value for event ${event.EventId}: ${error.message}`, {
          error,
          server,
          event,
        });
