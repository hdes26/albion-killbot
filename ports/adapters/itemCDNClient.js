...
async function downloadFromCDNs(item, writer) {
  console.log(`Searching item "${item.Type}_Q${item.Quality}" in CDNs...`, { item });
  ...
  for (const cdn of CDNS) {
    ...
    try {
      ...
      return new Promise((resolve) => {
        console.log(`Downloading "${url}"...`, { item, cdn });
      ...
      console.log(`Unable to download ${url}: ${error.message}`, { error });
    }
  }
