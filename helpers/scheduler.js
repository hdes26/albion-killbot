...
const clean = () => {
  ...
  for (const timeoutId of timeoutIds) {
    console.log(`Cleaning timeout: ${timeoutId}`);
    clearTimeout(timeoutId);
    timeoutIds.delete(timeoutId);
  }

  for (const cronjob of cronjobs) {
    console.log(`Halting cronjob: ${cronjob.id}`);
    cronjob.stop();
...
async function execFn(name, fn, ...fnOpts) {
  try {
    ...
    console.log(`An error ocurred in routine "${name}":`, e);
  }
}
...
async function runInterval(name, fn, { fnOpts = [], interval = 30000, runOnStart = false }) {
  ...
  console.log(`Scheduling interval function: ${name}`, {
    name,
    interval,
    runOnStart,
  });
...
function runCronjob(name, cronTime, fn, { fnOpts = [], runOnStart = false }) {
  ...
  console.log(`Scheduling cronjob function: ${name}`, {
    name,
    cronTime,
    runOnStart,
  });
