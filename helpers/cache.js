...
const get = (key, { debug = false } = {}) => {
  if (!cache.has(key)) {
    if (debug) console.log(`Cache miss: ${key}. Cache size: ${cache.size}`);
    return null;
  }
  ...
  if (data.expires && data.expires < Date.now()) {
    remove(key);
    if (debug) console.log(`Cache expired: ${key}. Cache size: ${cache.size}`);
    return null;
  }


  if (debug) console.log(`Cache hit: ${key}. Cache size: ${cache.size}`);
  return data.value;
};
...
const set = (key, value, { timeout, timeoutCallback, debug = false, ignore } = {}) => {
  ...
  if (debug) console.log(`Cache set: ${key}. Cache size: ${cache.size}`);
...
const memoize = async (key, fn, { timeout, timeoutCallback, debug, ignore, refresh } = {}) => {
  ...
  if (refresh) {
    ...
    data.intervalId = setInterval(async () => {
      ...
      if (debug) console.log(`Cache refresh: ${key}. Cache size: ${cache.size}`);
    }, refresh);
